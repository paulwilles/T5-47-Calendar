    ...
    for (int i = 0; i < schedule->item_count; ++i) {
        ...
        if (item->all_day || strcmp(item->start_label, "Any") == 0 || item->start_label[0] == '\0') {
            if (item->continues_from_prev_day && item->continues_next_day) {
                snprintf(time_buf, sizeof(time_buf), "All day (cont.)");
        ...
        } else if (item->continues_from_prev_day && item->continues_next_day) {
            snprintf(time_buf, sizeof(time_buf), "<- continues ->");
        } else if (item->continues_from_prev_day) {
            snprintf(time_buf, sizeof(time_buf), "<- %s", item->end_label[0] ? item->end_label : "");
        } else if (item->continues_next_day) {
            snprintf(time_buf, sizeof(time_buf), "%s ->", item->start_label[0] ? item->start_label : "");
        } else {
...
static void log_right_pane(const app_state_t *state)
{
    if (state->mode == UI_MODE_DETAIL) {
        ...
        if (item->all_day || strcmp(item->start_label, "Any") == 0 || item->start_label[0] == '\0') {
            if (item->continues_from_prev_day && item->continues_next_day) {
                ESP_LOGI(TAG, "  Time     : All day (continues)");
            } else if (item->continues_from_prev_day) {
                ESP_LOGI(TAG, "  Time     : <- All day");
            } else if (item->continues_next_day) {
                ESP_LOGI(TAG, "  Time     : All day ->");
            } else {
                ESP_LOGI(TAG, "  Time     : All day");
            ...
            ESP_LOGI(TAG, "  Time     : <- continues ->");
        } else if (item->continues_from_prev_day) {
            ESP_LOGI(TAG, "  Time     : <- %s", item->end_label[0] ? item->end_label : "");
        } else if (item->continues_next_day) {
            ESP_LOGI(TAG, "  Time     : %s ->", item->start_label[0] ? item->start_label : "");
        } else {
...
void app_main(void)
{
    ...
    const bool ext1_wake   = (wakeup == ESP_SLEEP_WAKEUP_EXT1);  /* WAKE (NEXT) button */
    const bool ext0_wake   = (wakeup == ESP_SLEEP_WAKEUP_EXT0);  /* SELECT button */
    const bool button_wake = ext0_wake || ext1_wake;

    app_state_t *state = &s_state;
    memset(state, 0, sizeof(*state));

    ESP_LOGI(TAG, "== T5 Calendar == wake: %s",
             first_boot ? "first-boot" : timer_wake ? "timer" :
             ext1_wake ? "wake-btn" : "select-btn");
    ...
    /* Restore timezone on every boot — TZ env var is RAM-only and lost across deep sleep */
    ...
    if (timer_wake) {
        /* Timer wake: update clock + sleep button strip only — no data fetch, no full render */
        char datetime_buf[24];
        ...
        /* Preserve RTC-restored detail selection across the placeholder snapshot.
         * The placeholder may have zero items and would force mode=overview,
         * causing BACK->SEL and day navigation after wake. */
        if (state->snapshot.day_count > 0) {
            if (state->selected_day_index < 0) {
                state->selected_day_index = 0;
            }
        ...
        if (ext1_wake) {
            /* Quick top-bar update first so PREV/NEXT buttons appear immediately */
            char datetime_buf_fast[24];
            {
                time_t tnow = time(NULL);
...
#if APP_ENABLE_WIFI
        /* Both SELECT and WAKE wakes need live data so navigation renders correctly */
        ...
        if (ext0_wake) {
            state_apply_button(state, BUTTON_ACTION_SELECT);
        }
        /* For ext1_wake: EPD already shows the calendar; just open interactive window.
         * render_dashboard will be called when user presses a button. */
        if (!ext1_wake) {
            render_dashboard(state);
        }
        ...
        /* Before sleep: update top bar to show WAKE button, hide PREV */
        {
            char datetime_buf[24];
            time_t tnow = time(NULL);
            struct tm *tm_info = localtime(&tnow);
    ...
    ESP_LOGI(TAG, "Deep sleep for %ds (SELECT or WAKE early)", APP_SLEEP_REFRESH_S);
